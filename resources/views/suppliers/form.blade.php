        ...
        <form method="POST" action="{{ $supplier->exists ? route('suppliers.update', $supplier) : route('suppliers.store') }}">
            ...
            <div class="card-body">
                <div class="row g-3">
                    ...
                    <div class="col-md-8">
                        <label class="form-label" for="nama">{{ __('wky.medan.nama_syarikat') }} <span class="text-danger">*</span></label>
                    ...
                    <div class="col-md-6">
                        <label class="form-label" for="pegawai_perhubungan">{{ __('wky.medan.pegawai_perhubungan') }}</label>
                    ...
                    <div class="col-md-6">
                        <label class="form-label" for="telefon">{{ __('wky.medan.telefon') }}</label>
                        <input class="form-control" id="telefon" name="telefon" value="{{ old('telefon', $supplier->telefon) }}">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label" for="emel">{{ __('wky.medan.emel') }}</label>
                    ...
                    <div class="col-md-6 d-flex align-items-end">
                        <div class="form-check form-switch">
                            ...
                            <label class="form-check-label" for="aktif">{{ __('wky.pembekal.pembekal_aktif') }}</label>
                        </div>
                    </div>
                    <div class="col-12">
                        <label class="form-label" for="alamat">{{ __('wky.medan.alamat') }}</label>
                        <textarea class="form-control" id="alamat" name="alamat" rows="3">{{ old('alamat', $supplier->alamat) }}</textarea>
                    </div>
                </div>
            </div>
            <div class="card-footer d-flex gap-2">
                <button class="btn btn-primary" type="submit">{{ $supplier->exists ? __('wky.aksi.kemas_kini') : __('wky.aksi.simpan') }}</button>
                <a class="btn btn-outline-secondary" href="{{ route('suppliers.index') }}">{{ __('wky.aksi.batal') }}</a>
            </div>
        </form>
    </div>
@endsection
